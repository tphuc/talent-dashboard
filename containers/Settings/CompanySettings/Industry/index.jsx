...
import { useCRUDIndustry } from "services/crud/useCRUDIndustry";
...
const Form = ({ title, onSubmit = async () => { }, defaultValues }) => {
    ...
    return <Panel css={{ padding: "$4" }}>
        ...
        <form onSubmit={handleSubmit(_onSubmit)}>
            <Card>
                <Input errorMessage={formState.errors.name?.message} {...register('name')} label='Name'></Input>
            </Card>
            <br />
...
export default function SettingIndustry() {


    const { data, mutate } = useGetIndustries();
        ...
        _action: <div>
            <Dialog modal>
                ...
                <DialogContent css={{ maxWidth: 400 }} >
                    <Form title={'Edit industry'} onSubmit={async (values) => onEdit(item.id, values)} defaultValues={{
                        name: item.name
                    }}></Form>
                </DialogContent>
            </Dialog>
    ...
    const { create, update, remove } = useCRUDIndustry();
    ...
    return <div>
        ...
        <span><b>Company Industries</b></span>
        ...
        <DropdownMenu  >
            ...
            <Dialog modal>
                ...
                <DialogContent css={{ maxWidth: 400 }} >
                    <Form title='Add add industry' onSubmit={onAdd}></Form>
                </DialogContent>
            </Dialog>
        </div>
