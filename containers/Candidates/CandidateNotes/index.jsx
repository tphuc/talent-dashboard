...
import { useGetCandidateNotes } from "services/swr/useGetCandidateNotes";
import { useCRUDCandidateNote } from "services/crud/useCRUDCandidateNote";
...
export default function CandidateNotesTab() {
    ...
    const { data, mutate } = useGetCandidateNotes(id);
    const toast = useToasts()

    const { create, update, remove } = useCRUDCandidateNote();
    ...
    return <div>
        <Column>
            ...
            <PaginatedList items={data} renderItem={(item, id) =>
                <CandidateNoteItem key={item.id} item={item} actions={<div>
                    <Dialog modal>
                        <DialogTrigger asChild>
