...
export default function CreateNewCompany() {
    ...
    const { create: createNewCompany } = useCRUDCompany();
    ...
    const onSubmit = async (vals) => {
        ...
        try{
            let res = await createNewCompany(values)
            console.log(res.data, res.status)
            if(res.data.status){
                toast.add('Add company successfully')
    ...
    return <Box css={{ padding: 20 }}>
        <Button onClick={() => router.push('/companies')} css={{ background: "transparent", mb: "$1" }} startEnhancer={<ArrowLeftIcon />}>All companies</Button>
        <br />
            ...
            <Panel>
                ...
                <Card>
                    <Box css={{
                        ...
                        <div></div>
                        <div>
                            <Select onSelectionChange={(val) => setValue('industry_id', val)} css={{ width: "100%" }} placeholder='select industry' label='Industry'>
                                {industries?.map((item, id) => <Select.Item key={item.id} value={item.id} textValue={item.name}>{item.name}</Select.Item>)}
                            </Select>
                        </div>
                        <div>
                        ...
                        <Input {...register('company_name')}  label='Company name' />

                        <Input {...register('company_email')}  label='Company email' />
                        <Input {...register('company_phone')}  label='Company phone' />
...
CreateNewCompany.getLayout = (page) => {
